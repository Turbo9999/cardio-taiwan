...
async function requireAdmin(req, serviceKey) {
  ...
  const profileResponse = await fetch(`${SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(user.id)}&select=is_admin`, {
...
async function rest(serviceKey, path, options = {}) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, {
    ...options,
    headers: serviceHeaders(serviceKey, options.headers),
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "members") {
      ...
      const { response, payload } = await rest(serviceKey, `profiles?select=id,display_name,xp,streak_days,created_at&order=created_at.desc&limit=100${filter}`, {
    ...
    if (action === "delete_member") {
      ...
      await Promise.all([
        rest(serviceKey, `profiles?id=eq.${encodeURIComponent(id)}`, { method: "DELETE" }),
        rest(serviceKey, `workouts?user_id=eq.${encodeURIComponent(id)}`, { method: "DELETE" }),
        rest(serviceKey, `user_badges?user_id=eq.${encodeURIComponent(id)}`, { method: "DELETE" }),
      ]);
    ...
    if (action === "clear_statistics") {
      await Promise.all([
        rest(serviceKey, "workouts?id=not.is.null", { method: "DELETE" }),
        rest(serviceKey, "search_events?id=not.is.null", { method: "DELETE" }),
        rest(serviceKey, "profiles?id=not.is.null", { method: "PATCH", body: JSON.stringify({ xp: 0, streak_days: 0 }) }),
      ]);
      return res.status(200).json({ success: true });
    }
